...
from .numpy_vjps import untake, balanced_eq, match_complex, replace_zero, dot_adjoint_0, dot_adjoint_1, tensordot_adjoint_0, tensordot_adjoint_1, nograd_functions
from ..extend import defjvp, defjvp_argnum, def_linear, vspace, JVPNode, register_notrace
...
defjvp(anp._array_from_scalar_or_array, None, None, lambda g, ans, args, kwargs, _: anp._array_from_scalar_or_array(args, kwargs, g))
...
defjvp(anp.maximum, lambda g, ans, x, y: g * balanced_eq(x, ans, y), lambda g, ans, x, y: g * balanced_eq(y, ans, x))
defjvp(anp.minimum, lambda g, ans, x, y: g * balanced_eq(x, ans, y), lambda g, ans, x, y: g * balanced_eq(y, ans, x))
defjvp(anp.fmax, lambda g, ans, x, y: g * balanced_eq(x, ans, y), lambda g, ans, x, y: g * balanced_eq(y, ans, x))
defjvp(anp.fmin, lambda g, ans, x, y: g * balanced_eq(x, ans, y), lambda g, ans, x, y: g * balanced_eq(y, ans, x))
...
defjvp(anp.power, lambda g, ans, x, y: g * y * x ** anp.where(y, y - 1, 1.0), lambda g, ans, x, y: g * anp.log(replace_zero(x, 1.0)) * ans)
...
defjvp(anp.fabs, lambda g, ans, x: anp.sign(x) * g)
...
defjvp(anp.where, None, lambda g, ans, c, x=None, y=None: anp.where(c, g, anp.zeros(anp.shape(g))), lambda g, ans, c, x=None, y=None: anp.where(c, anp.zeros(g.shape), g))
...
defjvp(anp.linspace, lambda g, ans, start, stop, *args, **kwargs: anp.linspace(g, 0, *args, **kwargs), lambda g, ans, start, stop, *args, **kwargs: anp.linspace(0, g, *args, **kwargs))
