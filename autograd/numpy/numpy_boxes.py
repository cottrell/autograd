...
import numpy as np
from ..extend import Box, primitive
...
for type_ in [float, np.float64, np.float32, np.float16, complex, np.complex64, np.complex128]:
    ArrayBox.register(type_)
nondiff_methods = ['all', 'any', 'argmax', 'argmin', 'argpartition', 'argsort', 'nonzero', 'searchsorted', 'round']
diff_methods = ['clip', 'compress', 'cumprod', 'cumsum', 'diagonal', 'max', 'mean', 'min', 'prod', 'ptp', 'ravel', 'repeat', 'reshape', 'squeeze', 'std', 'sum', 'swapaxes', 'take', 'trace', 'transpose', 'var']
