...
class VJPNode(Node):
    ...
    def __init__(self, value, fun, args, kwargs, parent_argnums, parents):
        ...
        try:
            ...
            raise NotImplementedError('VJP of {} wrt argnums {} not defined'.format(fun_name, parent_argnums))
...
def defvjp(fun, *vjpmakers, **kwargs):
    ...
    vjps_dict = {argnum: translate_vjp(vjpmaker, fun, argnum) for (argnum, vjpmaker) in zip(argnums, vjpmakers)}
...
defvjp(func(VSpace.inner_prod), None, lambda ans, vs, x, y: lambda g: vs.covector(vs.scalar_mul(y, g)), lambda ans, vs, x, y: lambda g: vs.covector(vs.scalar_mul(x, g)))
defvjp(func(VSpace.covector), None, lambda ans, vs, x: lambda g: vs.covector(g))
defvjp(func(VSpace.scalar_mul), None, lambda ans, vs, x, a: lambda g: vs.covector(vs.scalar_mul(vs.covector(g), a)), lambda ans, vs, x, a: lambda g: vs.inner_prod(g, vs.covector(x)))
