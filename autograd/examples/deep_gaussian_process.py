...
def build_deep_gp(input_dimension, hidden_dimension, covariance_function):
    (num_params_layer1, predict_layer1, log_marginal_likelihood_layer1) = make_gp_funs(covariance_function, num_cov_params=input_dimension + 1)
    (num_params_layer2, predict_layer2, log_marginal_likelihood_layer2) = make_gp_funs(covariance_function, num_cov_params=hidden_dimension + 1)
    ...
    def log_marginal_likelihood(all_params):
        ...
        return log_marginal_likelihood_layer1(layer1_params, X, h) + log_marginal_likelihood_layer2(layer2_params, np.atleast_2d(h).T, y)
    predict_layer_funcs = [predict_layer1, predict_layer2]
    return (total_num_params, log_marginal_likelihood, combined_predict_fun, unpack_all_params, predict_layer_funcs)
if __name__ == '__main__':
    n_data = 20
    input_dimension = 1
    hidden_dimension = 1
    (X, y) = build_step_function_dataset(D=input_dimension, n_data=n_data)
    (total_num_params, log_marginal_likelihood, combined_predict_fun, unpack_all_params, predict_layer_funcs) = build_deep_gp(input_dimension, hidden_dimension, rbf_covariance)
    ...
    def plot_gp(ax, X, y, pred_mean, pred_cov, plot_xs):
        ...
        ax.fill(np.concatenate([plot_xs, plot_xs[::-1]]), np.concatenate([pred_mean - 1.96 * marg_std, (pred_mean + 1.96 * marg_std)[::-1]]), alpha=0.15, fc='Blue', ec='None')
        rs = npr.RandomState(0)
