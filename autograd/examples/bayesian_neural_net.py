...
def make_nn_funs(layer_sizes, L2_reg, noise_variance, nonlinearity=np.tanh):
    ...
    def unpack_layers(weights):
        ...
        for (m, n) in shapes:
            yield (weights[:, :m * n].reshape((num_weight_sets, m, n)), weights[:, m * n:m * n + n].reshape((num_weight_sets, 1, n)))
...
if __name__ == '__main__':
    ...
    (num_weights, predictions, logprob) = make_nn_funs(layer_sizes=[1, 20, 20, 1], L2_reg=0.1, noise_variance=0.01, nonlinearity=rbf)
    (inputs, targets) = build_toy_dataset()
    log_posterior = lambda weights, t: logprob(weights, inputs, targets)
    (objective, gradient, unpack_params) = black_box_variational_inference(log_posterior, num_weights, num_samples=20)
