...
from .. import grad
from ..misc.optimizers import adam
...
def build_mog_bbsvi(logprob, num_samples, k=10, rs=npr.RandomState(0)):
    ...
    def sample(var_mixture_params, num_samples, rs):
        ...
        samples = np.concatenate([component_sample(params_k, num_samples, rs)[:, np.newaxis, :] for params_k in var_params], axis=1)
    ...
    def mixture_log_density(var_mixture_params, x):
        ...
        (log_weights, var_params) = unpack_mixture_params(var_mixture_params)
        component_log_densities = np.vstack([component_log_density(params_k, x) for params_k in var_params]).T
...
if __name__ == '__main__':
    ...
    (init_var_params, elbo, variational_log_density, variational_sampler) = build_mog_bbsvi(log_density, num_samples=40, k=10)
    ...
    variational_params = adam(grad(objective), init_var_params(D), step_size=0.1, num_iters=2000, callback=callback)