...
def expected_new_max(mean, std, max_so_far):
    return max_so_far - (mean - max_so_far) * norm.cdf(mean, max_so_far, std) + std * norm.pdf(mean, max_so_far, std)
...
def bayesian_optimize(func, domain_min, domain_max, num_iters=20, callback=None):
    ...
    def choose_next_point(domain_min, domain_max, acquisition_function, num_tries=15, rs=npr.RandomState(0)):
        ...
        def optimize_point(init_point):
            print('.', end='')
            result = minimize(grad_obj, x0=init_point, jac=True, method='L-BFGS-B', options={'maxiter': 10}, bounds=list(zip(domain_min, domain_max)))
...
if __name__ == '__main__':
    ...
    def callback(X, y, predict_func, acquisition_function, next_point, new_value):
        ...
        ax.fill(np.concatenate([plot_xs, plot_xs[::-1]]), np.concatenate([pred_mean - 1.96 * pred_std, (pred_mean + 1.96 * pred_std)[::-1]]), alpha=0.15, fc='Blue', ec='None')
        ax.plot(X, y, 'kx')
