...
def make_continuous(f, occlusion):
    ...
    num = np.roll(f, 1, axis=0) * np.roll(non_occluded, 1, axis=0) + np.roll(f, -1, axis=0) * np.roll(non_occluded, -1, axis=0) + np.roll(f, 1, axis=1) * np.roll(non_occluded, 1, axis=1) + np.roll(f, -1, axis=1) * np.roll(non_occluded, -1, axis=1)
    den = np.roll(non_occluded, 1, axis=0) + np.roll(non_occluded, -1, axis=0) + np.roll(non_occluded, 1, axis=1) + np.roll(non_occluded, -1, axis=1)
...
if __name__ == '__main__':
    ...
    result = minimize(objective_with_grad, init_occlusion, jac=True, method='CG', options={'maxiter': 50, 'disp': True}, callback=callback)
    ...
    os.system('convert -delay 5 -loop 0 step*.png -delay 250 step{0:03d}.png wing.gif'.format(simulation_timesteps))
