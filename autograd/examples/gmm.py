...
from .. import grad, hessian_vector_product
from scipy.optimize import minimize
from ..scipy.special import logsumexp
from ..scipy.stats import multivariate_normal as mvn
from ..misc.flatten import flatten_func
from data import make_pinwheel

def init_gmm_params(num_components, D, scale, rs=npr.RandomState(0)):
    return {'log proportions': rs.randn(num_components) * scale, 'means': rs.randn(num_components, D) * scale, 'lower triangles': np.zeros((num_components, D, D)) + np.eye(D)}
...
if __name__ == '__main__':
    ...
    def callback(flattened_params):
        ...
        plt.draw()
        plt.pause(1.0 / 60.0)
    minimize(flattened_obj, flattened_init_params, jac=grad(flattened_obj), hessp=hessian_vector_product(flattened_obj), method='Newton-CG', callback=callback)