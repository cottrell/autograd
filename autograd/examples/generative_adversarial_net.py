...
def init_random_params(scale, layer_sizes, rs=npr.RandomState(0)):
    ...
    return [(scale * rs.randn(m, n), scale * rs.randn(n)) for (m, n) in zip(layer_sizes[:-1], layer_sizes[1:])]
...
def adam_minimax(grad_both, init_params_max, init_params_min, callback=None, num_iters=100, step_size_max=0.001, step_size_min=0.001, b1=0.9, b2=0.999, eps=10 ** (-8)):
    ...
    for i in range(num_iters):
        ...
        m_max = (1 - b1) * g_max + b1 * m_max
        v_max = (1 - b2) * g_max ** 2 + b2 * v_max
        mhat_max = m_max / (1 - b1 ** (i + 1))
        vhat_max = v_max / (1 - b2 ** (i + 1))
        x_max = x_max + step_size_max * mhat_max / (np.sqrt(vhat_max) + eps)
        m_min = (1 - b1) * g_min + b1 * m_min
        v_min = (1 - b2) * g_min ** 2 + b2 * v_min
        mhat_min = m_min / (1 - b1 ** (i + 1))
...
if __name__ == '__main__':
    ...
    optimized_params = adam_minimax(both_objective_grad, init_gen_params, init_dsc_params, step_size_max=step_size_max, step_size_min=step_size_min, num_iters=num_epochs * num_batches, callback=print_perf)